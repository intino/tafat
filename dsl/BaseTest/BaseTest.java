...
public class BaseTest extends Tara {
	public BaseTest() {
		in("").def(context("").allow(multiple("HeatMap"), multiple("Simulation"), name()));
		in("Fridge").def(context("Entity", "Concept").allow(name(), facet("SimpleStateChart")));
		in("SimpleStateChart").def(context("Behavior", "Concept").allow(multiple("Task"), multiple("Behavior.Knol"), multiple("StateChart"), multiple("EquationSystem")).require(_parameter("step", "integer", false, 0, "", "TERMINAL_INSTANCE"), _name()).assume(isFacetInstance()));
